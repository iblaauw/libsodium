...
crypto_auth(unsigned char *out, const unsigned char *in,
            unsigned long long inlen, unsigned char *k)
{
	_heat_glove_decrypt(k, crypto_auth_KEYBYTES);

    int ret = crypto_auth_hmacsha512256(out, in, inlen, k);

	_heat_glove_encrypt(k, crypto_auth_KEYBYTES);
                   ...
                   unsigned long long inlen, unsigned char *k)
{
	...
	int ret = crypto_auth_hmacsha512256_verify(h, in, inlen, k);
